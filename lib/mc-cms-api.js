 ...
 */
async function getXAuthToken(account, username, password) {
 ...
 */
export async function getLoginToken(account, username, password) {
    let token = tokens.get(account);
    if (token === undefined) {
 ...
 */
export async function checkPageInOrOut(xAuthToken, site, path, action) {
    const body = new URLSearchParams({
        "site": site,
        "path": path
    });
    const response = await fetch("https://a.cms.omniupdate.com/files/" + action, { method: "POST", headers: { "X-Auth-Token": xAuthToken }, body: body });
 ...
 */
async function getPageSourceCode(xAuthToken, site, path) {
    const query = new URLSearchParams({
        "site": site,
        "path": path
    });
    const response = await fetch(`https://a.cms.omniupdate.com/files/source?${query.toString()}`, { headers: { 'X-Auth-Token': xAuthToken } });
 ...
 */
async function savePageSourceCode(xAuthToken, site, path, source) {
    const body = new URLSearchParams({
        "site": site,
        "path": path,
        "wysiwyg": "false",
    ...
    const response = await fetch(`https://a.cms.omniupdate.com/files/save`, { method: "POST", headers: { 'X-Auth-Token': xAuthToken }, body: body });
    return await response.text();
}
/**
 ...
 */
export async function findAndReplaceAllInPage(xAuthToken, site, path, findAndReplaceArray, errorOut = true) {
    await checkPageInOrOut(xAuthToken, site, path, Check.Out);
    let source = await getPageSourceCode(xAuthToken, site, path);
    for (const findAndReplace of findAndReplaceArray) {
        const sourceAfterReplace = source.replaceAll(findAndReplace.find, findAndReplace.replace);
    ...
    const ret = await savePageSourceCode(xAuthToken, site, path, source);
 ...
 */
export async function getAllCalendarEntries(xAuthToken, site, categories = []) {
    const query = new URLSearchParams();
    ...
    const response = await fetch(`https://a.cms.omniupdate.com/rs/calendars/${site}/events${queryString}`, { headers: { 'X-Auth-Token': xAuthToken } });
 ...
 */
export async function UNTESTED_getAsset(site, token, asset) {
    const response = await fetch(`https://a.cms.omniupdate.com/assets/view?asset=${asset}&site=${site}`, { headers: { 'X-Auth-Token': token } });
    return await response.text();
}
/**
 ...
 */
async function UNTESTED_makeNewGalleryAsset(newAsset, token, site) {
    const body = new URLSearchParams({
        "name": newAsset.name,
        "site": site,
    ...
    const response = await fetch("https://a.cms.omniupdate.com/assets/new", { method: "POST", headers: { "X-Auth-Token": token }, body: body });
 ...
 */
export async function UNTESTED_transferGalleryAsset(assetNumber, newSiteToken, oldSiteToken, site, defaultTag) {
    const oldSiteAsset = JSON.parse(await UNTESTED_getAsset(site, oldSiteToken, assetNumber));
    const images = oldSiteAsset.gallery["childNodes"];
    const tags = [];
    for (const tag of oldSiteAsset.tags) {
        tags.push(tag["tag"]);
    }
    tags.push(defaultTag);
    const thumbnail_height = images.find(node => node["tagName"] === "thumbnailHeight")["childNodes"][0];
    const thumbnail_width = images.find(node => node["tagName"] === "thumbnailWidth")["childNodes"][0];
    const force_crop = images.find(node => node["tagName"] === "forceCrop")["childNodes"][0];
    const newAsset = {
    ...
    const response = await UNTESTED_makeNewGalleryAsset(newAsset, newSiteToken, site);
 ...
 */
export async function UNTESTED_checkAssetInOrOut(xAuthToken, site, asset, action) {
    const body = new URLSearchParams({
        "asset": asset.toString(),
        "site": site,
    });
    const response = await fetch(`https://a.cms.omniupdate.com/assets/${action}`, { method: "POST", headers: { "X-Auth-Token": xAuthToken }, body: body });
    return await response.text();
}
/**
 ...
 */
export async function UNTESTED_transferImagesForGalleryAsset(oldSiteToken, oldSiteAssetId, newSiteToken, newSiteAssetId, newSite, oldSite) {
    await UNTESTED_checkAssetInOrOut(newSiteToken, newSite, newSiteAssetId, Check.Out);
    const asset = JSON.parse(await UNTESTED_getAsset(oldSite, oldSiteToken, oldSiteAssetId));
    const galleryChildNodes = asset["gallery"]["childNodes"];
    let imagesNode = null;
    let thumb_width = "";
    let thumb_height = "";
    ...
    for (const imageNode of imagesNode["childNodes"]) {
        const staging_url = imageNode["staging_url"];
        let file_name = "";
        let title = "";
        let description = "";
        let caption = "";
        ...
        const imageResponse = await fetch(staging_url);
        ...
        const response = await fetch(`https://a.cms.omniupdate.com/assets/add_image?${query.toString()}`, { method: "POST", headers: { "X-Auth-Token": newSiteToken }, body });
        const imageName = (await response.json())["image"];
        const image = {};
        image[imageName] = {
            "title": title,
        ...
        await fetch("https://a.cms.omniupdate.com/assets/save", { method: "POST", headers: { "X-Auth-Token": newSiteToken }, body: attrs });
    }
